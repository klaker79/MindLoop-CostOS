...
// BUMP v309 (albaran: paridad con la recepcion manual). Tres arreglos: (1) el aviso de duplicado viajaba como booleano y el modal esperaba el objeto -> el banner salia sin un solo dato ("...ya lo escaneaste: ."); ahora va el objeto entero (proveedor, fecha, lineas, factura). (2) Reescanear un albaran YA registrado soltaba "No pude cargar las lineas" (el listado solo devuelve lineas pendientes y el batch estaba aprobado); ahora se consulta GET /purchases/batch/:batchId y se muestra que se registro y cuando. (3) El proveedor se casaba por igualdad/contencion y fallaba con nombres reales ("OROSA AVES HUEVOS Y CAZA" vs "OROSA AVES Y HUEVOS"); ahora casa por PALABRAS en comun con umbral 0.6 y exigiendo una palabra de >=4 letras, para no confundir "PESCADOS PEREZ" con "PESCADOS GOMEZ".
// BUMP v310 (albaran ya registrado: avisar si el PEDIDO sigue pendiente). Que el albaran este registrado no significa que el pedido se recibiera: si entro como compra suelta, el pedido sigue 'pendiente' y el stock subio por otra via. Decir solo "ya esta registrado, no hace falta subirlo" era enganoso. Ahora se listan los pedidos pendientes de ese proveedor y se avisa de no recibirlos otra vez para no contar el stock dos veces.
const CACHE_NAME = 'mindloop-costos-v310';
