...
window.procesarArchivoVentas = async function (input) {
    ...
    try {
        ...
        if (isPDF) {
            ...
            const response = await fetch(`${window.API_CONFIG?.baseUrl ?? 'https://lite-api.mindloop.cloud'}/api/parse-pdf`, {
                method: 'POST',
                credentials: 'include',
...
window.cargarResumenMensual = async function () {
    ...
    try {
        ...
        const response = await fetch(
            ...
            `${window.API_CONFIG?.baseUrl ?? 'https://lite-api.mindloop.cloud'}/api/monthly/summary?mes=${mes}&ano=${ano}`,
            {
                credentials: 'include',
                headers: Object.assign({ 'Content-Type': 'application/json' }, window.authToken ? { 'Authorization': `Bearer ${window.authToken}` } : {}),
            }
...
window.actualizarKPIsDiario = async function () {
    ...
    try {
        ...
        const apiBase = window.API_CONFIG?.baseUrl ?? 'https://lite-api.mindloop.cloud';
