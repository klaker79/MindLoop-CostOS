    ...
    datos.forEach(item => {
        ...
        html += `
          <tr style="background: ${bgColor};">
            ...
            <td style="padding: 10px; text-align: right;">${item.precio.toFixed(2)}${window.currentUser?.moneda || '€'}</td>
    ...
    datos.forEach(item => {
        ...
        html += `
          <tr style="background: ${bgColor};">
            ...
            <td style="padding: 10px; text-align: right;">${item.precioVenta.toFixed(2)}${window.currentUser?.moneda || '€'}</td>
    ...
    datos.forEach(item => {
        ...
        html += `
          <tr style="background: ${bgColor};">
            ...
            <td style="padding: 10px; text-align: right;">${item.total.toFixed(2)}${window.currentUser?.moneda || '€'}</td>
          </tr>`;
    });
...
window.actualizarKPIsDiario = function () {
    ...
    if (elCompras) elCompras.textContent = totalCompras.toFixed(2) + ' ' + (window.currentUser?.moneda || '€');
    const elVentas = document.getElementById('diario-total-ventas');
    if (elVentas) elVentas.textContent = totalIngresos.toFixed(2) + ' ' + (window.currentUser?.moneda || '€');
    const elBeneficio = document.getElementById('diario-beneficio');
    if (elBeneficio) elBeneficio.textContent = beneficioBruto.toFixed(2) + ' ' + (window.currentUser?.moneda || '€');
...
function renderizarTablaComprasDiarias() {
    ...
    for (const [nombre, data] of Object.entries(ingredientes)) {
        ...
        dias.forEach(dia => {
            ...
            if (diaData) {
                ...
                html += `<td style="text-align: center; background: #FFF5F2; padding: 18px; border-right: 1px solid #E2E8F0;">
                    <div style="font-weight: 700; color: #1E293B; font-size: 1em;">${importeDia.toFixed(2)}${window.currentUser?.moneda || '€'}${avisoDescuento}</div>
                    <small style="color:#64748B;">${diaData.precio.toFixed(2)}${window.currentUser?.moneda || '€'}/${unidad} × ${diaData.cantidad}</small>
                </td>`;
            } else {
                html +=
        ...
        html += `<td style="text-align: center; background: #e8f5e9; font-weight: bold; padding: 18px;">${totalVisible.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
        html += '</tr>';
        rowIndex++;
    }
    html += '</tbody></table>';
    ...
    for (const [nombre, data] of Object.entries(recetas)) {
        ...
        dias.forEach(dia => {
            ...
            if (diaData) {
              ...
              <div style="color: #2e7d32; font-weight: 500;">${diaData.ingresos.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
        ...
        html += `<td style="text-align: center; background: #e8f5e9;">
          <div style="font-weight: bold;">${ingresosVisibles.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
...
function renderizarTablaProveedoresDiarios() {
    ...
    proveedoresOrdenados.forEach(([nombre, data], idx) => {
        ...
        dias.forEach(dia => {
            ...
            if (valor > 0) {
                ...
                html += `<div style="font-weight: 600; color: #1E293B; font-size: 0.95em;">${valor.toFixed(0)}${window.currentUser?.moneda || '€'}</div>`;
                html += '</td>';
            } else {
        ...
        html += `<td style="text-align: center; background: linear-gradient(135deg, #ECFDF5 0%, #D1FAE5 100%); font-weight: 700; padding: 14px 16px; color: #065F46; font-size: 1.05em;">${totalVisibleProv.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
        html += '</tr>';
    });
    ...
    dias.forEach(dia => {
        ...
        if (total > 0) {
            html += `<td style="text-align: center; padding: 14px 8px; border-right: 1px solid #E2E8F0; background: linear-gradient(135deg, #FFF7ED 0%, #FFEDD5 100%); font-weight: 700; color: #9A3412;">${total.toFixed(0)}${window.currentUser?.moneda || '€'}</td>`;
        } else {
            html += '<td style="text-align: center; color: #CBD5E1; padding: 14px 8px; border-right: 1px solid #E2E8F0;">-</td>';
        }
    });
    html += `<td style="text-align: center; background: linear-gradient(135deg, #DBEAFE 0%, #BFDBFE 100%); font-weight: 700; padding: 14px 16px; color: #1E40AF; font-size: 1.1em;">${totalGeneral.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    html += '</tr>';
...
async function renderizarTablaPLDiario() {
    ...
    dias.forEach(dia => {
        ...
        totalIngresos += val;
        html += `<td style="text-align: center; padding: 16px 8px; font-weight: 600; color: #166534; border-bottom: 1px solid #bbf7d0;">${val.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    });
    html += `<td style="text-align: center; background: #1e40af; color: white; font-weight: 700; padding: 16px;">${totalIngresos.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
    ...
    dias.forEach(dia => {
        ...
        totalCostes += val;
        html += `<td style="text-align: center; padding: 16px 8px; color: #dc2626; border-bottom: 1px solid #fecaca;">${val.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    });
    html += `<td style="text-align: center; background: #1e40af; color: white; font-weight: 700; padding: 16px;">${totalCostes.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
    ...
    dias.forEach(dia => {
        ...
        html += `<td style="text-align: center; padding: 16px 8px; font-weight: 700; color: ${color}; border-bottom: 1px solid #fcd34d;">${margenDia.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    });
    html += `<td style="text-align: center; background: #1e40af; color: white; font-weight: 700; padding: 16px;">${totalMargenBruto.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
    ...
    dias.forEach(() => {
        html += `<td style="text-align: center; padding: 16px 8px; color: #be185d; border-bottom: 1px solid #f9a8d4;">${gastosFijosDia.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    });
    html += `<td style="text-align: center; background: #1e40af; color: white; font-weight: 700; padding: 16px;">${totalGastosFijosMostrados.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
    ...
    dias.forEach(dia => {
        ...
        html += `<td style="text-align: center; padding: 18px 8px; font-weight: 700; font-size: 14px; color: ${color}; background: ${bg}; border-bottom: 2px solid #93c5fd;">${beneficioNeto.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
    });
    const totalBeneficioNeto = totalMargenBruto - totalGastosFijosMostrados;
    const colorTotal = totalBeneficioNeto >= 0 ? '#22c55e' : '#ef4444';
    html += `<td style="text-align: center; background: linear-gradient(135deg, #1e40af 0%, #1d4ed8 100%); color: ${colorTotal}; font-weight: 800; font-size: 16px; padding: 18px; text-shadow: 0 1px 2px rgba(0,0,0,0.2);">${totalBeneficioNeto.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
    ...
    <div style="margin-top: 24px; padding: 20px; background: linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%); border-radius: 12px; border: 1px solid #fcd34d;">
        ...
        <div style="display: flex; gap: 24px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 200px;">
                ...
                <div style="font-size: 28px; font-weight: 700; color: #d97706;">${totalCompras.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
            </div>
    ...
    dias.forEach(dia => {
        ...
        if (val > 0) {
            html += `<span style="background: white; padding: 4px 10px; border-radius: 6px; font-size: 12px; border: 1px solid #fcd34d;">${fecha.getDate()}/${fecha.getMonth() + 1}: <strong style="color: #d97706;">${val.toFixed(2)}${window.currentUser?.moneda || '€'}</strong></span>`;
        }
    });
