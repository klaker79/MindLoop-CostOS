    ...
    (window.ingredientes || []).forEach(ing => {
        if (ing.nombre?.toLowerCase().includes(q) || ing.categoria?.toLowerCase().includes(q)) {
            matches.push({
                ...
                icon: '🥬',
                title: ing.nombre,
                subtitle: `${escapeHTML(ing.categoria || t('common:no_category'))} • ${parseFloat(ing.precio || 0).toFixed(2)}${window.getCurrencySymbol?.() || '€'}/${escapeHTML(ing.unidad || 'ud')}`,
                action: () => {
    ...
    (window.recetas || []).forEach(rec => {
        if (rec.nombre?.toLowerCase().includes(q) || rec.codigo?.toString().includes(q)) {
            matches.push({
                ...
                icon: '🍽️',
                title: rec.nombre,
                subtitle: `${escapeHTML(rec.categoria || t('common:no_category'))} • PVP: ${parseFloat(rec.precio_venta || 0).toFixed(2)}${window.getCurrencySymbol?.() || '€'}`,
                action: () => {
    ...
    (window.pedidos || []).forEach(ped => {
        ...
        if (provNombre.toLowerCase().includes(q) || ped.estado?.toLowerCase().includes(q)) {
            matches.push({
                ...
                subtitle: `${escapeHTML(ped.estado || 'pendiente')} • ${parseFloat(ped.total || 0).toFixed(2)}${window.getCurrencySymbol?.() || '€'}`,
                action: () => {
