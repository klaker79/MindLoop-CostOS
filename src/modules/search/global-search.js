...
export function initGlobalSearch() {
    ...
    searchContainer.innerHTML = `
        <div style="position: relative;">
            ...
            >
            <span class="gs-kbd-hint" style="position: absolute; right: 12px; top: 50%; transform: translateY(-50%); color: #94a3b8; font-size: 11px; background: #f1f5f9; padding: 2px 6px; border-radius: 4px; border: 1px solid #e2e8f0;">⌘K</span>
        </div>
    ...
    (window.recetas || []).forEach(rec => {
        if (rec.nombre?.toLowerCase().includes(q) || rec.codigo?.toString().includes(q)) {
            matches.push({
                ...
                icon: '🍽️',
                title: rec.nombre,
                subtitle: `${escapeHTML(rec.categoria || t('common:no_category'))} • PVP: ${cm(parseFloat(rec.precio_venta || 0))}`,
                action: () => {
    ...
    (window.pedidos || []).forEach(ped => {
        ...
        if (provNombre.toLowerCase().includes(q) || ped.estado?.toLowerCase().includes(q)) {
            matches.push({
                ...
                subtitle: `${escapeHTML(ped.estado || 'pendiente')} • ${cm(parseFloat(ped.total || 0))}`,
                action: () => {
