    ...
    try {
        ...
        if (select) {
            const options = recetasOrdenadas.map(r => {
                ...
                return `<option value="${r.id}" data-search="${escapeHTML(r.nombre.toLowerCase())}">${escapeHTML(r.nombre)} - ${precio.toFixed(2)}${window.currentUser?.moneda || '€'}</option>`;
            }).join('');
      ...
      `;
        ...
        Object.keys(ventasPorFecha)
            ...
            .forEach(fecha => {
                ...
                html += `<tr style="background:#F8FAFC;"><td colspan="6" style="padding:12px 16px;font-weight:600;color:#1E293B;border-bottom:1px solid #E2E8F0;">${fecha} - ${t('common:label_total')}: ${totalDia.toFixed(2)}${window.currentUser?.moneda || '€'}</td></tr>`;
                ...
                ventasDia.forEach(v => {
                    ...
                    html += `<tr><td style="padding:8px 16px 8px 32px;color:#64748B;">${fecha}</td><td style="padding:8px 16px;color:#64748B;">${hora}</td><td style="padding:8px 16px;color:#1E293B;">${escapeHTML(v.receta_nombre || '')}</td><td style="padding:8px 16px;text-align:center;color:#64748B;">${v.cantidad}</td><td style="padding:8px 16px;text-align:right;"><strong style="color:#1E293B;">${parseFloat(v.total).toFixed(2)}${window.currentUser?.moneda || '€'}</strong></td><td style="padding:8px 16px;text-align:center;"><button class="icon-btn delete" onclick="window.eliminarVenta(${v.id})" title="${t('common:btn_delete')}">🗑️</button></td></tr>`;
                });
            });
    ...
    try {
        ...
        variantes.forEach(v => {
            html += `<option value="${v.id}" data-factor="${v.factor}" data-precio="${v.precio_venta}">
                ${escapeHTML(v.nombre)} - ${parseFloat(v.precio_venta).toFixed(2)}${window.currentUser?.moneda || '€'} (${v.factor}x)
            </option>`;
        });
