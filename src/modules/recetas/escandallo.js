...
export async function verEscandallo(recetaId) {
    ...
    document.getElementById('escandallo-resumen').innerHTML = `
        <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px;">
            <div style="background: #F0FDF4; padding: 12px; border-radius: 8px; text-align: center;">
                ...
                <div style="font-size: 20px; font-weight: 700; color: #10B981;">${costeTotal.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
            </div>
            <div style="background: #EFF6FF; padding: 12px; border-radius: 8px; text-align: center;">
                ...
                <div style="font-size: 20px; font-weight: 700; color: #3B82F6;">${precioVenta.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
            </div>
            <div style="background: #FEF3C7; padding: 12px; border-radius: 8px; text-align: center;">
                <div style="font-size: 11px; color: #64748B; text-transform: uppercase;">${t('recetas:escandallo_margin')}</div>
                <div style="font-size: 20px; font-weight: 700; color: #F59E0B;">${margenEuros.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
            </div>
        ...
        <div style="margin-top: 16px; background: linear-gradient(135deg, #1E293B 0%, #334155 100%); border-radius: 12px; padding: 20px; color: #E2E8F0;">
            ...
            <div style="display: grid; grid-template-columns: 1fr auto; gap: 6px 16px; font-size: 13px;">
                ...
                <span style="text-align: right; font-weight: 600;">${costeTotal.toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #94A3B8;">Margen de error (${MARGEN_ERROR_PCT}%)</span>
                <span style="text-align: right; font-weight: 600; color: #FBBF24;">+${margenError.toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #F8FAFC; font-weight: 700; padding-top: 6px; border-top: 1px solid #475569;">Subtotal coste / ración</span>
                <span style="text-align: right; font-weight: 700; color: #F8FAFC; padding-top: 6px; border-top: 1px solid #475569;">${subtotalConError.toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #94A3B8; padding-top: 8px;">Precio sugerido (${precioIdealLabel} FC)</span>
                <span style="text-align: right; font-weight: 600; padding-top: 8px; color: #67E8F9;">${precioIdeal.toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #94A3B8;">Beneficio bruto</span>
                <span style="text-align: right; font-weight: 600; color: ${beneficioBruto >= 0 ? '#34D399' : '#F87171'};">${beneficioBruto.toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #94A3B8;">IVA (${IVA_PCT}%)</span>
                <span style="text-align: right; font-weight: 600;">${(precioVenta * IVA_PCT / 100).toFixed(2)}${window.currentUser?.moneda || '€'}</span>

                <span style="color: #F8FAFC; font-weight: 700; font-size: 15px; padding-top: 8px; border-top: 1px solid #475569;">PVP (IVA incl.)</span>
                <span style="text-align: right; font-weight: 700; font-size: 15px; color: #F97316; padding-top: 8px; border-top: 1px solid #475569;">${pvpConIva.toFixed(2)}${window.currentUser?.moneda || '€'}</span>
    ...
    desglose.forEach((item, i) => {
        ...
        tablaHtml += `<td style="text-align: right; padding: 5px; border-bottom: 1px solid #E2E8F0; font-weight: 600;">${item.coste.toFixed(2)}${window.currentUser?.moneda || '€'}</td>`;
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function (context) {
                            ...
                            return ` ${item.coste.toFixed(2)}${window.currentUser?.moneda || '€'}  ·  ${item.porcentaje.toFixed(1)}%`;
                        }
                    }
                }
            },
...
export async function exportarPDFEscandallo() {
    ...
    const summaryData = [
        { label: t('recetas:escandallo_cost'), value: `${costeTotal.toFixed(2)}${window.currentUser?.moneda || '€'}`, color: [16, 185, 129] },
        { label: t('recetas:escandallo_pvp'), value: `${precioVenta.toFixed(2)}${window.currentUser?.moneda || '€'}`, color: [59, 130, 246] },
        { label: t('recetas:escandallo_margin'), value: `${margenEuros.toFixed(2)}${window.currentUser?.moneda || '€'}`, color: [245, 158, 11] },
    ...
    desglose.forEach((item, i) => {
        ...
        doc.text(`${item.precioUnitario.toFixed(2)}${window.currentUser?.moneda || '€'}/${item.unidad}`, 118, tableY + 5);
        doc.setFont('helvetica', 'bold');
        doc.text(`${item.coste.toFixed(2)}${window.currentUser?.moneda || '€'}`, 153, tableY + 5);
        doc.setFont('helvetica', 'normal');
    ...
    doc.text(`${costeTotal.toFixed(2)}${window.currentUser?.moneda || '€'}`, 153, tableY + 5.5);
