...
export function agregarIngredienteReceta() {
    ...
    ingredientesOrdenados.forEach(ing => {
        ...
        optionsHtml += `<option value="${ing.id}">${escapeHTML(ing.nombre)} (${precio}${window.getCurrencySymbol?.() || '€'}/${escapeHTML(unidad)})</option>`;
    });
    ...
    if (recetasBase.length > 0) {
        ...
        recetasBase.forEach(rec => {
            ...
            optionsHtml += `<option value="rec_${rec.id}" data-es-receta="true">🧪 ${escapeHTML(rec.nombre)} (${coste.toFixed(2)}${window.getCurrencySymbol?.() || '€'})</option>`;
        });
    }
...
export function calcularCosteReceta() {
    ...
    if (costeDiv) {
        ...
        if (costeSpan) costeSpan.textContent = costeTotal.toFixed(2) + (window.getCurrencySymbol?.() || '€');
...
export async function renderizarRecetas() {
    ...
    `;
        ...
        recetasPagina.forEach(rec => {
            ...
            html += `<td>${coste.toFixed(2)} ${window.getCurrencySymbol?.() || '€'}</td>`;
            html += `<td>${rec.precio_venta ? parseFloat(rec.precio_venta).toFixed(2) : '0.00'} ${window.getCurrencySymbol?.() || '€'}</td>`;
            html += `<td><span class="badge ${badgeClass}">${margen.toFixed(2)} ${window.getCurrencySymbol?.() || '€'} (${pct}%)</span></td>`;
...
export function actualizarPrecioSugerido() {
    ...
    const coste = parseFloat(costeText.replace(/[€A-Z]+/gi, '').replace(',', '.')) || 0;
    ...
    precioSpan.textContent = precioSugerido.toFixed(2) + (window.getCurrencySymbol?.() || '€');
}
...
export function aplicarPrecioSugerido() {
    ...
    const precio = parseFloat(precioText.replace(/[€A-Z]+/gi, '').replace(',', '.'));
