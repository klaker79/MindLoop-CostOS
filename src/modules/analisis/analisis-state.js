...
export async function getFoodCostCanonical({ force = false, desde, hasta } = {}) {
    ...
    // ⚠️ La cache va INDEXADA por periodo: el food cost DEPENDE del rango
    // (mes 31,5% vs 90d 34,2% en La Nave 5). Sin la clave, un segundo caller
    // con otro rango dentro del TTL recibía en silencio el número de un
    // periodo distinto — justo la clase de descuadre prohibida
    // (feedback_datos_cuadran_entre_pestanas). Auditoría 2026-07-08.
    const cacheKey = `${desde}|${hasta}`;
    if (!force && !isExpired(state.pnl) && state.pnl.key === cacheKey) return state.pnl.data;
    try {
        ...
        state.pnl = { data: val, ts: Date.now(), key: cacheKey };
        return val;
    } catch (e) {
