    ...
    return `<div class="intel-list">${data.recetas_problema.slice(0, 6).map(r => `
        <div class="intel-item">
            <div>
                ...
                <div class="intel-item-detail">Coste: ${r.coste.toFixed(2)}${window.currentUser?.moneda || '€'} · Actual: ${r.precio_actual.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
            </div>
            <span class="intel-badge badge-danger">${r.food_cost}% → ${r.precio_sugerido.toFixed(2)}${window.currentUser?.moneda || '€'}</span>
        </div>
    `).join('')}</div>`;
}
...
function renderWaste(data) {
    ...
    let html = `
        <div style="text-align:center;margin-bottom:16px;">
            <div style="font-size:2.5rem;font-weight:700;color:#ef4444;">${total.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
        ...
        topProductos.forEach(p => {
            html += `
                <div class="intel-item">
                    ...
                    <span class="intel-badge badge-danger">${parseFloat(p.perdida_total).toFixed(2)}${window.currentUser?.moneda || '€'}</span>
                </div>
            `;
        });
        html += `</div>`;
