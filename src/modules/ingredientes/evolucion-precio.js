...
export async function verEvolucionPrecio(ingredienteId) {
    ...
    document.getElementById('evolucion-summary').innerHTML = `
        <div style="background: #ecfdf5; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #047857;">${cm(precioMin)}</div>
        </div>
        <div style="background: #f0f9ff; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #0369a1;">${cm(precioPromedio)}</div>
        </div>
        <div style="background: #fef2f2; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #b91c1c;">${cm(precioMax)}</div>
        </div>
    `;
...
function renderTablaHistorial() {
    ...
    itemsMostrar.forEach((h, i) => {
        ...
        tableHtml += `<tr>
            ...
            <td style="padding: 8px; text-align: right; font-weight: 600;">${cm(h.precio)}</td>
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    displayColors: false,
                    callbacks: {
                        label: (ctx) => `💰 ${cm(ctx.parsed.y)} / ${ingrediente.unidad}`
                    }
                }
            },
            scales: {
