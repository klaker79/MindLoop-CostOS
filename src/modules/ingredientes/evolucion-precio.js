...
export async function verEvolucionPrecio(ingredienteId) {
    ...
    document.getElementById('evolucion-summary').innerHTML = `
        <div style="background: #ecfdf5; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #047857;">${precioMin.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
        </div>
        <div style="background: #f0f9ff; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #0369a1;">${precioPromedio.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
        </div>
        <div style="background: #fef2f2; padding: 15px; border-radius: 10px; text-align: center;">
            ...
            <div style="font-size: 24px; font-weight: 700; color: #b91c1c;">${precioMax.toFixed(2)}${window.currentUser?.moneda || '€'}</div>
        </div>
    `;
...
function renderTablaHistorial() {
    ...
    itemsMostrar.forEach((h, i) => {
        ...
        tableHtml += `<tr>
            ...
            <td style="padding: 8px; text-align: right; font-weight: 600;">${h.precio.toFixed(2)}${window.currentUser?.moneda || '€'}</td>
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: {
                    ...
                    displayColors: false,
                    callbacks: {
                        label: (ctx) => `💰 ${ctx.parsed.y.toFixed(2)}${window.currentUser?.moneda || '€'} / ${ingrediente.unidad}`
                    }
                }
            },
            scales: {
                y: {
                    ...
                    ticks: {
                        callback: (value) => value.toFixed(2) + (window.currentUser?.moneda || '€'),
                        color: '#94A3B8',
