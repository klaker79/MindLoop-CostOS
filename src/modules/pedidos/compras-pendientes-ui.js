            ...
            ">
                ...
                <div style="background: #fee2e2; border: 1px solid #fca5a5; border-radius: 10px; padding: 10px 14px; margin-bottom: 12px; display: flex; align-items: center; gap: 10px;">
                    ...
                    <div>
                        ...
                        <div style="font-size: 12px; color: #b91c1c;">${fechaPedidoDup}${totalPedidoDup ? ` · ${totalPedidoDup}${window.currentUser?.moneda || '€'}` : ''} · ${t('pedidos:pending_status')}: ${estadoPedidoDup}</div>
                    </div>
                </div>` : ''}
...
function recalcularTotalBatch(elementInside) {
    ...
    totalEl.textContent = `Total: ${total.toFixed(2)}${window.currentUser?.moneda || '€'}`;
}
