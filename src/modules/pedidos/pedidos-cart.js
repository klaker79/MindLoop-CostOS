...
window.agregarAlCarrito = function (ingredienteId, cantidad = 1, proveedorId = null, precioProveedor = null, esUnidadSuelta = false) {
    ...
    if (!precioFormato && provId && window.ingredientesProveedores) {
        ...
        if (rel && rel.precio) {
            ...
            console.log(`💰 Precio de proveedor ${provId} para ${ing.nombre}: ${precioFormato}${window.currentUser?.moneda || '€'}/unidad`);
        }
    }
...
function renderizarCarrito() {
    ...
    `;
        document.getElementById('carrito-total').textContent = '0.00 ' + (window.currentUser?.moneda || '€');
        return;
    }
    ...
    `;
        ...
        <tr style="border-bottom: 1px solid #e2e8f0;">
          ...
          <td style="padding: 12px; text-align: right;">${item.precio.toFixed(2)} ${window.currentUser?.moneda || '€'}</td>
          <td style="padding: 12px; text-align: right; font-weight: bold;">${subtotal.toFixed(2)} ${window.currentUser?.moneda || '€'}</td>
          <td style="padding: 12px; text-align: center;">
    ...
    document.getElementById('carrito-total').textContent = total.toFixed(2) + ' ' + (window.currentUser?.moneda || '€');
}
